...
ua, cz, pl, jp, en = Local("localization/ua.json"), Local("localization/cz.json"), Local("localization/pl.json"), Local("localization/jp.json"), Local("localization/en.json")
status = [False, "debug-line"]

# Функція отримання локалізації
def getcurlenglocal(tag, lang=None):
    # Якщо мова вказана, використовуємо її для отримання локалізації
    if lang:
        if lang == "ua":
            return ua.get(tag)
        elif lang == "en":
            ...
            return jp.get(tag)
    # Якщо мова не задана, використовуємо глобальну змінну current_lang
    else:
        global current_lang
        return ua.get(tag) if current_lang == "ua" else \
            en.get(tag) if current_lang == "en" else \
            cz.get(tag) if current_lang == "cz" else \
            pl.get(tag) if current_lang == "pl" else \
            jp.get(tag)  # Повертаємо для японської мови



# Ініціалізація додатку
App = QApplication(sys.argv)
App.setApplicationName("Ban_ivan-coin")
...
pllocalization = QCheckBox(getcurlenglocal("pl-localization"))
jplocalization = QCheckBox(getcurlenglocal("jp-localization"))
...
def localization():
    ...
    try:
        ...
        if current_lang == "cz":
            current_lang = "ua"  # Якщо зараз чеська, перемикаємо на українську
            ...
            current_lang = "cz"  # Якщо зараз японська, перемикаємо на чеську
        else:
            current_lang = "cz"  # За замовчуванням чеська
        ...
        pllocalization.setText(getcurlenglocal("pl-localization"))
        jplocalization.setText(getcurlenglocal("jp-localization"))
...
def menu_func(q):
    match q.text():
        ...
        case "Vylepšení":
            pumping_opne()
        case "ゲームを終了":
            quit_game_func()  # Японський текст "Вийти з гри"
        case "ログインに戻る":
            go_login()  # Японський текст "Вийти в меню входу"
        case "受け入れる":
            del_account_accept()  # Японський текст "Прийняти"
        case "ブースト":
            boosts_open()  # Японський текст "Бусти"
        case "ポンプ":
            pumping_opne()  # Японський текст "Прокачка"
        case _:
            print(q.text())  # Якщо текст не співпав, вивести його
